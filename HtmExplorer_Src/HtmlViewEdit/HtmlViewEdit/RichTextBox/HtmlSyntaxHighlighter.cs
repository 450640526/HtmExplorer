 using System; using System.Text; using System.Drawing;  /// <summary> /// https://gist.github.com/ArthurHub/ee8f3d143d656d5a836a/download# /// HTML syntax highlighting using Rich-Text formatting.<br/> /// - Handle plain input or already in RTF format.<br/> /// - Handle if input already contains RTF color table.<br/> /// - Rich coloring adjusted to Visual Studio HTML coloring.<br/> /// - Support to provide custom colors.<br/> /// - High performance (as much as RTF format allows).<br/> /// </summary> /// <remarks> /// The MIT License (MIT) Copyright (c) 2014 Arthur Teplitzki.<br/> /// Based on work by Alun Evans 2006 (http://www.codeproject.com/Articles/15038/C-Formatting-Text-in-a-RichTextBox-by-Parsing-the). /// </remarks>  public class HtmlSyntaxHighlighter {
    ...
    private static string Process(string text, string colorScheme)
    {
        ...
        for (; i < text.Length; i++)
        {
            ...
            {
                sb.Append("\\cf4").Append(c).Append("\\cf4 ");
            }
            else if (inHtmlTag && !inComment && inAttributeVal && c == ':')
            {
            ...
            {
                sb.Append("\\cf0").Append(c).Append("\\cf4 ");
            }
